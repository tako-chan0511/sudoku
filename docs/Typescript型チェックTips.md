...
```
**原因:**  
Composition APIのdefinePropsで、リテラル型を伴う独自の型（例: Stuffが 'item' | 'box' | 'area' のユニオン型）をPropType経由で指定した場合、型が正しく絞り込まれない。
...
```
**原因:**  
オプショナルなPropsを定義したつもりでも、型推論上は必須になってしまい、親側で省略するとエラーになる。
...
`const user = ref<User>()` のように初期値を省略すると `Ref<User|undefined>` 型となり、テンプレートで型エラーが出ることがあります。
